...
import { UPLOAD_IMAGE_URL, UPLOAD_PORTFOLIO_IMAGE_1_URL } from '../../config/configuration';
...
class UploadPhoto extends Component {
    ...
    onDrop = (picture) => {
        let idName = (this.props.customIdName)? this.props.customIdName : "imgUploader";
        let blob = document.getElementById(idName).files[0];
        let formData = new FormData();
        
        formData.append("imgUploader", blob);
        this.props.onLoaderChange(true);
        let url = (this.props.uploadUrl) ? this.props.uploadUrl : UPLOAD_IMAGE_URL;
        sendPostRequest(url, formData, true, this.props.user.token).then((_res) => {
        ...
        return (
            <div>
                ...
                <div className="upload-btn-wrapper">
                    ...
                    <input type="file" id={(this.props.customIdName)? this.props.customIdName : "imgUploader"} onChange={(e) => this.onDrop(e)} />
                </div>
            </div>
        );
    }
